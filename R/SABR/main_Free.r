...
# this script uses 1st approach to calibrate SABR parameters by fitting
# all three parameters, rho, nu, alpha together.
rm(list=ls())
graphics.off()

source("SABRVol.r")
source("FitSABRFree.r")
...
beta1 <- 1
res1 <- FitSABRFree(beta1, f, K, maturity, obs_vols)
alpha1 <- res1$parms[1]
rho1 <- res1$parms[2]
nu1 <- res1$parms[3]

beta2 <- .5
res2 <- FitSABRFree(beta2, f, K, maturity, obs_vols)
alpha2 <- res2$parms[1]
rho2 <- res2$parms[2]
nu2 <- res2$parms[3]

beta3 <- 0
res3 <- FitSABRFree(beta3, f, K, maturity, obs_vols)
alpha3 <- res3$parms[1]
rho3 <- res3$parms[2]
nu3 <- res3$parms[3]
